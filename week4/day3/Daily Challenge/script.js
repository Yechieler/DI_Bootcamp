...
// function getCarHonda(carInventory, model) {
//     const filter = carInventory.filter(item => {
//         return item.car_make == model
//     });
//     return `This is a ${filter[0].car_make} ${filter[0].car_model} from ${filter[0].car_year}.`;

// }
// console.log(getCarHonda(inventory, 'Honda'));
...
let carYear = carInventory.map(item => item.car_year)
...
for (let i = 0; i < inventory.length; i++) {
    for (let j = 0; j < inventory.length; j++) {
        // console.log("i"+i+"j"+j);
        if (inventory[j].car_year == carYear[i]){
...
// const sorter = carInventory.sort(item => {
